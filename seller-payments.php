...
<html>
	<head>
		<title>FullCashback Seller Payments - Get Full Cashback!</title>
		<meta charset="utf-8" />
	...
	<section class="py-5 text-center container">
		<div class="row py-lg-5">
		  <div class="col-lg-6 col-md-8 mx-auto">
			<h1 class="fw-light">My Payments</h1>
			<br>
		  </div>
		</div>
	  </section>
	  ...
	  <div class="album py-5 bg-light">
		...
		<table class="table table-hover table-striped">
		  ...
		  <?php
			...
			$sql = "SELECT * FROM payments WHERE selleremail='$selleremail'";
			...
			if ($result->num_rows > 0) {
			  ...
			  while($row = $result->fetch_assoc()) {
				  ...
				  $buyerusername[$row['productid']] = $row['buyerpaypalusername'];
			  }
			} else {
			...
			?>
					  ...
					  <tr>
						  ...
						  <?php if ($purchaseverified[$row_['id']] == "false") { ?>
						  <form method="POST" action="verify-purchase.php">
							<input type="hidden" name="paymentid" value="<?php echo $paymentids[$row_['id']] ?>">
						    <td><button type="submit" class="btn btn-primary">Verify Purchase</button></td>
						  </form>
						  <?php } else {?>
						  <td>true</td>
						  <?php } ?>
						  
						  <?php if ($paymentreceived[$row_['id']] == "false") { ?>
						    <td><a href="https://paypal.me/<?php echo $buyerusername[$row_['id']] ?>/<?php echo $row_['price'] ?>"><button type="submit" class="btn btn-primary">Pay Cashback</button></a></td>
						  <?php } else {?>
						  <td>true</td>
						  <?php } ?>
						</tr>
